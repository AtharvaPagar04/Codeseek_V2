...
- `CODESEEK_GRAPH_RETRIEVAL_SHADOW=true` — Exposes repo knowledge graph diagnostics (shadow mode). Safe for production.
- `CODESEEK_GRAPH_RETRIEVAL_ACTIVE=false` — Global graph-active injection for every request. Keep `false` in production.
- `CODESEEK_GRAPH_ASSIST_USER_TOGGLE=true|false` — Allows the UI toggle for per-request Graph Assist opt-in. The local dev script (`scripts/run_local_backend.sh`) defaults this to `true`. Production `.env.example` defaults to `false`; enable after validation.
- `CODESEEK_GRAPH_ACTIVE_MAX_ADDED=2` — Maximum graph-active candidates injected per request.
- `CODESEEK_GRAPH_ACTIVE_MIN_SCORE=90` — Minimum graph candidate score threshold.
