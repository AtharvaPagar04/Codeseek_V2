...
from retrieval.api_service import QueryRequest, _build_query_diagnostics


def test_query_request_graph_retrieval_mode_defaults_and_validates():
    assert QueryRequest(query="test").graph_retrieval_mode == "standard"
    assert QueryRequest(query="test", graph_retrieval_mode="standard").graph_retrieval_mode == "standard"
    assert (
        QueryRequest(query="test", graph_retrieval_mode="graph_assist").graph_retrieval_mode
        == "graph_assist"
    )
    with pytest.raises(Exception):
        QueryRequest(query="test", graph_retrieval_mode="always_on")
...
def test_build_query_diagnostics_compacts_safe_fields():
    meta = {
        ...
        "graph_active": {
            ...
            "disabled_reason": "disabled_by_request",
            "added_count": 0,
            "added_chunks": [],
        },
        "display_sources": [
    ...
    assert diagnostics["graph_active"]["requested_mode"] == "standard"
    assert diagnostics["graph_active"]["effective_enabled"] is False
    assert diagnostics["graph_active"]["disabled_reason"] == "disabled_by_request"
