...
def isolated_query_stream_db(tmp_path, monkeypatch):
    monkeypatch.setenv("CODESEEK_DB_BACKEND", "sqlite")
    monkeypatch.setenv("CODESEEK_SQLITE_PATH", str(tmp_path / "query-stream.db"))
...
def test_query_stream_unauthorized():
    async def request():
        ...
        return await api_service.query_stream_v1(
            api_service.QueryRequest(query="test"),
            raw_request,
            authorization=None,
            x_request_id=None,
            session_token=None,
    ...
    with patch("retrieval.api_service._current_auth_user", return_value=mock_user), \
        ...
        async def request():
            ...
            async with httpx.AsyncClient(
                transport=transport,
                base_url="http://test",
                cookies={"codeseek_session": "dummy"},
            ) as client:
        ...
        assert [event["type"] for event in events] == [
            "status", "delta", "delta", "final_answer", "sources", "done"
        ]
        assert all(event["protocol_version"] == 2 for event in events)
        assert len({event["request_id"] for event in events}) == 1
        assert events[0]["message"] == "Retrieving..."
        assert events[1]["text"] == "Hello "
        assert events[2]["text"] == "world!"
        assert events[3]["text"] == "Hello world!"
        assert events[3]["message_id"] == "msg-1"
        assert events[4]["sources"] == []
        assert events[4]["message_id"] == "msg-1"
        assert events[4]["diagnostics"]["memory"]["history_injected"] is False
        assert events[4]["diagnostics"]["retrieval"]["candidate_count"] == 0
        assert events[5]["message_id"] == "msg-1"
        ...
        assert captured_run_query_kwargs["graph_retrieval_mode"] == "graph_assist"
...
def test_query_stream_abort():
    ...
    with patch("retrieval.api_service._current_auth_user", return_value=mock_user), \
        ...
        async def request_then_disconnect():
            request = Request(
                {
                    "type": "http",
                    "method": "POST",
            ...
            response = await api_service.query_stream_v1(
                api_service.QueryRequest(query="hello", session_id="session-123"),
                request,
                authorization=None,
                x_request_id="abort-request",
                session_token="dummy",
            )
        ...
        asyncio.run(request_then_disconnect())
