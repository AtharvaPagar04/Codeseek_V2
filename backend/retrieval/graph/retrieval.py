...
from retrieval.config import (
    get_collection_name,
    get_graph_active_config,
    get_graph_assist_config,
    get_graph_shadow_config,
)
...
def resolve_graph_active_request(requested_mode: str | None = None) -> dict[str, object]:
    """Resolve request-scoped Graph Assist against server-side kill switches."""
    mode = (requested_mode or GRAPH_RETRIEVAL_MODE_STANDARD).strip().lower()
    if mode not in {GRAPH_RETRIEVAL_MODE_STANDARD, GRAPH_RETRIEVAL_MODE_ASSIST}:
        mode = GRAPH_RETRIEVAL_MODE_STANDARD

    active_config = get_graph_active_config()
    assist_config = get_graph_assist_config()
    global_active_enabled = bool(active_config.get("enabled"))
    user_toggle_allowed = bool(assist_config.get("user_toggle_enabled"))
    graph_assist_requested = mode == GRAPH_RETRIEVAL_MODE_ASSIST
    graph_active_effective = global_active_enabled or (
        graph_assist_requested and user_toggle_allowed
    )

    disabled_reason = ""
    if not graph_active_effective:
        disabled_reason = (
            "disabled_by_server_config"
            if graph_assist_requested
            else "disabled_by_request"
        )

    return {
        "requested_mode": mode,
        "graph_assist_requested": graph_assist_requested,
        "user_toggle_allowed": user_toggle_allowed,
        "global_active_enabled": global_active_enabled,
        "graph_active_effective": graph_active_effective,
        "disabled_reason": disabled_reason,
    }


def annotate_graph_active_diagnostics(
    diagnostics: dict,
    decision: dict,
    graph_shadow: dict | None,
) -> dict:
    """Attach request-mode diagnostics without changing active selection gates."""
    enriched = dict(diagnostics or {})
    requested_mode = str(decision.get("requested_mode") or GRAPH_RETRIEVAL_MODE_STANDARD)
    graph_active_effective = bool(decision.get("graph_active_effective"))
    shadow = graph_shadow if isinstance(graph_shadow, dict) else {}
    shadow_status = str(shadow.get("status") or "").strip()

    disabled_reason = str(decision.get("disabled_reason") or "").strip()
    selector_reason = str(enriched.get("reason") or "").strip()
    if graph_active_effective and shadow_status != "ready":
        disabled_reason = "graph_shadow_not_ready"
    elif graph_active_effective and selector_reason in {"shadow_disabled"}:
        disabled_reason = "graph_shadow_not_ready"
    elif graph_active_effective and selector_reason.startswith("graph_shadow_"):
        disabled_reason = "graph_shadow_not_ready"

    enriched["requested_mode"] = requested_mode
    enriched["graph_assist_requested"] = bool(decision.get("graph_assist_requested"))
    enriched["user_toggle_allowed"] = bool(decision.get("user_toggle_allowed"))
    enriched["global_active_enabled"] = bool(decision.get("global_active_enabled"))
    enriched["effective_enabled"] = bool(
        graph_active_effective
        and enriched.get("enabled") is True
        and not disabled_reason
    )
    if disabled_reason:
        enriched["disabled_reason"] = disabled_reason
    else:
        enriched.pop("disabled_reason", None)
    return enriched
