...
from retrieval.stores.retrieval_trace_store import persist_retrieval_trace_from_query_result
...
class QueryRequest(BaseModel):
    ...
    graph_retrieval_mode: Literal["standard", "graph_assist"] = "standard"
...
def _persist_retrieval_trace_safely(
    *,
    session: dict | None,
    thread: dict | None,
    ...
    if not session or not thread or not user_message or not assistant_message:
        return
    try:
        persist_retrieval_trace_from_query_result(
        ...
        log_event(
            "api.query.retrieval_trace_persist_failed",
            request_id,
    ...
    try:
        ...
        if session:
            ...
            if thread:
                ...
                assistant_msg = append_thread_message(
                    thread["id"],
                    session["id"],
                    "assistant",
                    answer,
