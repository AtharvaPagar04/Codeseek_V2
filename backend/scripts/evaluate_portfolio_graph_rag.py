...
- Request standard retrieval or per-request Graph Assist:
  python backend/scripts/evaluate_portfolio_graph_rag.py --graph-retrieval-mode graph_assist
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=("standard", "graph_assist"),
        default="standard",
        help="Per-request graph retrieval mode to send to the backend.",
    )
...
def evaluate_case(case: dict[str, Any], raw_result: dict[str, Any], raw_path: Path) -> dict[str, Any]:
    ...
    graph_active_requested_mode = extract_value(graph_active, "requested_mode")
    graph_active_effective_enabled = extract_value(graph_active, "effective_enabled")
    graph_active_disabled_reason = extract_value(graph_active, "disabled_reason")
    ...
    return {
        ...
        "graph_active_requested_mode": graph_active_requested_mode,
        "graph_active_effective_enabled": graph_active_effective_enabled,
        "graph_active_reason": graph_active_reason,
        "graph_active_disabled_reason": graph_active_disabled_reason,
...
def render_markdown(
    *,
    ...
    lines = [
        ...
        f"- graph_retrieval_mode: `{runtime['graph_retrieval_mode']}`",
    ...
    for result in results:
        lines.extend(
            [
                ...
                f"- graph_active.requested_mode: {result['graph_active_requested_mode']}",
                f"- graph_active.effective_enabled: {result['graph_active_effective_enabled']}",
                f"- graph_active.reason: {result['graph_active_reason']}",
                f"- graph_active.disabled_reason: {result['graph_active_disabled_reason']}",
...
def main() -> int:
    ...
    for case in cases:
        ...
        raw_result = query_backend(
            ...
            case=case,
            timeout=args.timeout,
            graph_retrieval_mode=args.graph_retrieval_mode,
        )
