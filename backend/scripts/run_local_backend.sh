...
# Load .env — preserve PATH to prevent accidental clobbering by .env values.
SAVED_PATH="$PATH"
if [[ -f .env ]]; then
  set -a
...
# Graph Assist — local dev enables the user toggle so the UI can opt in.
# Graph active global default stays false; shadow provides diagnostics.
export CODESEEK_GRAPH_RETRIEVAL_SHADOW="${CODESEEK_GRAPH_RETRIEVAL_SHADOW:-true}"
export CODESEEK_GRAPH_RETRIEVAL_ACTIVE="${CODESEEK_GRAPH_RETRIEVAL_ACTIVE:-false}"
export CODESEEK_GRAPH_ASSIST_USER_TOGGLE="${CODESEEK_GRAPH_ASSIST_USER_TOGGLE:-true}"
export CODESEEK_GRAPH_ACTIVE_MAX_ADDED="${CODESEEK_GRAPH_ACTIVE_MAX_ADDED:-2}"
export CODESEEK_GRAPH_ACTIVE_MIN_SCORE="${CODESEEK_GRAPH_ACTIVE_MIN_SCORE:-90}"
...
export CODESEEK_DESCRIPTION_MAX_TOKENS="${CODESEEK_DESCRIPTION_MAX_TOKENS:-1024}"
...
echo "[local-backend] starting backend (port ${PORT:-8000})"
echo "[local-backend] graph shadow=${CODESEEK_GRAPH_RETRIEVAL_SHADOW}  active=${CODESEEK_GRAPH_RETRIEVAL_ACTIVE}  user_toggle=${CODESEEK_GRAPH_ASSIST_USER_TOGGLE}  max_added=${CODESEEK_GRAPH_ACTIVE_MAX_ADDED}  min_score=${CODESEEK_GRAPH_ACTIVE_MIN_SCORE}"

export PYTHONPATH="${BACKEND_ROOT}${PYTHONPATH:+:$PYTHONPATH}"
exec ./.venv/bin/python -m uvicorn retrieval.api_service:app --host 127.0.0.1 --port "${PORT:-8000}"
