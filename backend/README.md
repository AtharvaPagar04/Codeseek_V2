...
Choose the profile matching your runtime target:
...
## Dependency Architecture & Docker Build Profiles

The CodeSeek backend supports modular dependency installations depending on the target infrastructure (CPU vs. GPU).

### Structure of Dependency Profiles

*   **`requirements-base.txt`**: Contains runtime packages common to all environments (e.g. FastAPI, Uvicorn, tree-sitter, psycopg, etc.). It excludes PyTorch/sentence-transformers.
*   **`requirements-cpu.txt`**: Production profile for CPU-only deployments. Restricts PyTorch to the CPU-only package build using the `--extra-index-url https://download.pytorch.org/whl/cpu` registry.
*   **`requirements-gpu.txt`**: Production profile for GPU-accelerated deployments. Installs standard CUDA-enabled PyTorch.
*   **`requirements-dev.txt`**: Development profile. Inherits CPU profile and adds quality assurance and testing packages (e.g. `pytest`).

### Docker Image Building

The backend `Dockerfile` uses a build argument (`BUILD_PROFILE`) to select the correct dependency file at compile-time:

#### CPU Build (Default / Recommended)
```bash
docker build \
  --build-arg BUILD_PROFILE=cpu \
  -t codeseek-backend-cpu:latest .
```

#### GPU Build
```bash
docker build \
  --build-arg BUILD_PROFILE=gpu \
  -t codeseek-backend-gpu:latest .
```
