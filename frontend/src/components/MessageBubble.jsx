...
export default function MessageBubble({ message, sessionId = '', userQuery = '', onOpenRetrievalTrace = null }) {
  ...
  const canOpenRetrievalTrace = !isUser && !message.loading && !message.error && message.id && typeof onOpenRetrievalTrace === 'function';
  ...
  const handleCopyDiagnostics = (e) => {
    ...
    const sections = ['Intent', 'Model', 'Sources', 'Graph', 'Validation', 'Freshness'];
    ...
    <div className="flex justify-start animate-fadeIn group">
      <div className="max-w-[90%] min-w-0">
          ...
          <div className="flex items-center gap-2 text-2xs text-text-muted mt-1 pl-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
              ...
              {canOpenRetrievalTrace && (
                <button
                  onClick={() => onOpenRetrievalTrace({ assistantMessageId: message.id })}
                  className="inline-flex items-center gap-1 rounded-full border border-border bg-surface-3 px-2 py-0.5 font-mono text-text-secondary transition-colors hover:border-cyan-500/40 hover:text-cyan-100"
                  title="Open the retrieval trace for this answer"
                  aria-label="Trace retrieval"
                >
                  <TraceIcon />
                  Trace retrieval
                </button>
              ...
              {diagnosticsRows.length > 0 && (
                <details className="group" ref={diagnosticsRef}>
                  ...
                  <div className="mt-3 space-y-4">
                    {['Intent', 'Model', 'Sources', 'Graph', 'Validation', 'Freshness'].map((sectionName) => {
...
function TraceIcon() {
  return (
    <svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true">
      ...
      <path d="M4.75 4h6.5M4.1 5.35 7 10.45M11.9 5.35 9 10.45" strokeLinecap="round" />
    </svg>
  );
}
