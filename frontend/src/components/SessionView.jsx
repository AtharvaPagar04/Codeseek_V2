...
const RepoGraphView = React.lazy(() => import('./graph/RepoGraphView'));
import { useChat } from '../hooks/useChat';
import {
  getGraphAssistEnabled,
  graphRetrievalModeForEnabled,
  setGraphAssistEnabled,
} from '../utils/graphAssist';
...
function GraphAssistToggle({ enabled, disabled, onChange }) {
  return (
    ...
    >
      ...
      <span
        aria-hidden="true"
        className={`relative inline-flex h-3 w-5 shrink-0 rounded-full border transition-colors ${enabled
            ? 'bg-text-primary border-text-primary'
            : 'bg-surface-1 border-border'
          }`}
      >
        <span
          className={`absolute top-0.5 h-1.5 w-1.5 rounded-full transition-transform ${enabled
              ? 'translate-x-3 bg-[#0a0a0a]'
      ...
      {/* Hover tooltip */}
      <span
        className="pointer-events-none absolute left-1/2 top-full z-50 mt-2 -translate-x-1/2 whitespace-nowrap rounded-md border border-border bg-surface-3 px-2 py-1 text-[10px] font-mono text-text-primary opacity-0 shadow-lg transition-opacity duration-150 group-hover:opacity-100 group-focus-within:opacity-100"
      >
        Graph assisted response
      </span>
    </label>
  );
}
...
export default function SessionView({
  ...
  const [graphAssistEnabled, setGraphAssistEnabledState] = useState(() =>
    getGraphAssistEnabled(session.id)
  );
  const [viewMode, setViewMode] = useState('chat'); // 'chat' | 'graph'
  const [graphRequestedMode, setGraphRequestedMode] = useState(null);
  const [traceSourceMode, setTraceSourceMode] = useState('latest');
  const [selectedTraceMessageId, setSelectedTraceMessageId] = useState(null);
  ...
  useEffect(() => {
    ...
    setViewMode('chat');
    setGraphRequestedMode(null);
    setTraceSourceMode('latest');
  ...
  useEffect(() => {
    setGraphAssistEnabledState(getGraphAssistEnabled(session.id));
  }, [session.id]);

  const handleGraphAssistChange = (enabled) => {
    setGraphAssistEnabledState(enabled);
    setGraphAssistEnabled(session.id, enabled);
  };

  const handleOpenRetrievalTrace = useCallback(({ assistantMessageId } = {}) => {
    setGraphRequestedMode('retrieval_trace');
    setTraceSourceMode(assistantMessageId ? 'message' : 'latest');
    setSelectedTraceMessageId(assistantMessageId || null);
    setViewMode('graph');
    onCloseSidebar?.();
  }, [onCloseSidebar]);

  const handleTraceSourceChange = useCallback(({ sourceMode, assistantMessageId } = {}) => {
    setTraceSourceMode(sourceMode === 'message' && assistantMessageId ? 'message' : 'latest');
    setSelectedTraceMessageId(sourceMode === 'message' && assistantMessageId ? assistantMessageId : null);
  }, []);
  ...
  const handleSend = () => {
    ...
    sendMessage(session, text, {
      graphRetrievalMode: graphRetrievalModeForEnabled(graphAssistEnabled),
    });
  };
      ...
      <div className="shrink-0 flex items-center justify-between px-6 py-2 bg-surface-2/20 border-b border-border backdrop-blur-md z-10">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`w-7 h-7 flex items-center justify-center rounded-full border transition-all duration-150 mr-1.5 shrink-0 ${showMetadata
              ? 'bg-surface-3 border-text-muted text-text-primary'
          ...
          {/* Chat / Graph tab switcher */}
          {isReady && (
            <div className="flex items-center bg-surface-3 rounded-lg border border-border p-0.5 ml-1">
              <button
        ...
        <Suspense fallback={
          <div className="flex-1 flex items-center justify-center bg-base">
            <div className="text-center space-y-3">
              ...
              <p className="text-[11px] text-text-muted font-mono">Loading graph view…</p>
            </div>
          </div>
        }>
          <RepoGraphView
            sessionId={session.id}
            requestedMode={graphRequestedMode}
            traceSourceMode={traceSourceMode}
            selectedTraceMessageId={selectedTraceMessageId}
            onTraceSourceChange={handleTraceSourceChange}
            onAskAbout={(query) => {
              setViewMode('chat');
              setInput(query);
            }}
          ...
          <div className="w-full max-w-xl mt-8">
            ...
            >
              ...
              <GraphAssistToggle
                ...
                onChange={handleGraphAssistChange}
              />
              <textarea
                ref={textareaRef}
                value={input}
              ...
              <button
                ...
                className={`shrink-0 w-8 h-8 flex items-center justify-center rounded-full text-base transition-all duration-150 ${isLoading
              ...
              {(activeThread?.messages || []).map((msg, idx, arr) => {
                ...
                return (
                  <MessageBubble
                    ...
                    onOpenRetrievalTrace={handleOpenRetrievalTrace}
                  />
                );
              })}
              <div ref={bottomRef} />
  ...
  return (
    <div className="relative shrink-0 flex items-center" ref={dropdownRef}>
      ...
      >
        ...
        <svg
          className={`h-2.5 w-2.5 transform text-text-muted transition-transform duration-150 ${isOpen ? 'rotate-180' : ''
            }`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          ...
          <div className="max-h-48 overflow-y-auto">
            {presets.map((opt) => (
              <button
                ...
                className={`w-full text-left rounded-lg px-2.5 py-1.5 hover:bg-surface-3 transition-colors flex flex-col ${opt.value === activeModel ? 'bg-surface-3/50' : ''
                  }`}
              >
    ...
    <div className="w-full flex flex-col gap-3 font-mono text-xs select-none">
      ...
      {!loading && !error && preview && (
        <div className="flex flex-col gap-4">
          ...
          {blockMessage && (
            <div className={`p-3 rounded-lg flex items-start gap-2 border text-[10px] ${blockType === 'error'
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-2">
                  <button
                    ...
                    className={`px-3 py-2 rounded-lg font-mono text-[10px] uppercase tracking-wider font-semibold transition-all flex items-center justify-center gap-1.5 ${isButtonEnabled && !isTriggering
