...
test('querySession invokes the correct endpoint and includes credentials/auth headers', async () => {
  ...
  try {
    ...
    assert.equal(payload.graph_retrieval_mode, 'standard');
  } finally {
...
test('querySession sends graph_assist retrieval mode when provided', async () => {
  const originalFetch = globalThis.fetch;
  let payload = null;
  ...
  try {
    ...
    assert.equal(payload.graph_retrieval_mode, 'graph_assist');
  } finally {
    globalThis.fetch = originalFetch;
    delete globalThis.localStorage;
  }
...
test('fetchGraphNodeDetails builds graph node details URL and includes credentials', async () => {
  const originalFetch = globalThis.fetch;
  let calledUrl = null;
  let calledOptions = null;
  ...
  try {
    const { fetchGraphNodeDetails } = await import('./api.js');
    const result = await fetchGraphNodeDetails('session 123', 'file|src/components/Projects.tsx');
    assert.equal(result.status, 'ready');
    assert.match(calledUrl, /\/api\/v1\/sessions\/session%20123\/graph\/node\/file%7Csrc%2Fcomponents%2FProjects\.tsx\/details$/);
    assert.equal(calledOptions.credentials, 'include');
  } finally {
    globalThis.fetch = originalFetch;
    delete globalThis.localStorage;
...
test('fetchGraphCodeBlock builds graph code block URL and returns parsed JSON', async () => {
  const originalFetch = globalThis.fetch;
  let calledUrl = null;
  let calledOptions = null;
  ...
  globalThis.fetch = async (url, options) => {
    ...
    return {
      ...
      json: async () => ({
        ...
        code: 'export function Projects() {}',
      }),
    };
  };

  try {
    const { fetchGraphCodeBlock } = await import('./api.js');
    const result = await fetchGraphCodeBlock('session 123', 'chunk|Projects');
    assert.equal(result.status, 'ready');
    assert.equal(result.code, 'export function Projects() {}');
    assert.match(calledUrl, /\/api\/v1\/sessions\/session%20123\/graph\/code-blocks\/chunk%7CProjects$/);
    assert.equal(calledOptions.credentials, 'include');
  } finally {
    globalThis.fetch = originalFetch;
    delete globalThis.localStorage;
...
test('fetchLatestRetrievalTrace builds retrieval trace URL and includes credentials', async () => {
  const originalFetch = globalThis.fetch;
  let calledUrl = null;
  let calledOptions = null;
  ...
  try {
    const { fetchLatestRetrievalTrace } = await import('./api.js');
    const result = await fetchLatestRetrievalTrace('session 123', { threadId: 'thread 1' });
    assert.equal(result.status, 'ready');
    assert.match(calledUrl, /\/api\/v1\/sessions\/session%20123\/retrieval-trace\/latest\?thread_id=thread\+1$/);
    assert.equal(calledOptions.credentials, 'include');
  } finally {
    globalThis.fetch = originalFetch;
    delete globalThis.localStorage;
...
test('fetchRetrievalTrace builds message-specific retrieval trace URL and includes credentials', async () => {
  const originalFetch = globalThis.fetch;
  let calledUrl = null;
  let calledOptions = null;
  ...
  try {
    const { fetchRetrievalTrace } = await import('./api.js');
    const result = await fetchRetrievalTrace('session 123', 'msg 123');
    assert.equal(result.trace_version, 'v2');
    assert.match(calledUrl, /\/api\/v1\/sessions\/session%20123\/retrieval-trace\/msg%20123$/);
    assert.equal(calledOptions.credentials, 'include');
  } finally {
    globalThis.fetch = originalFetch;
    delete globalThis.localStorage;
...
test('fetchRetrievalTraceMessages builds trace message list URL and includes credentials', async () => {
  const originalFetch = globalThis.fetch;
  let calledUrl = null;
  let calledOptions = null;
  ...
  globalThis.fetch = async (url, options) => {
    ...
    return {
      ...
      json: async () => ({
        session_id: 'session 123',
        items: [{ assistant_message_id: 'msg 123', trace_version: 'v2' }],
      }),
    };
  };

  try {
    const { fetchRetrievalTraceMessages } = await import('./api.js');
    const result = await fetchRetrievalTraceMessages('session 123', { limit: 12, threadId: 'thread 1' });
    assert.equal(result.items[0].assistant_message_id, 'msg 123');
    assert.match(calledUrl, /\/api\/v1\/sessions\/session%20123\/retrieval-trace\/messages\?/);
    assert.match(calledUrl, /limit=12/);
    assert.match(calledUrl, /thread_id=thread\+1|thread_id=thread%201/);
    assert.equal(calledOptions.credentials, 'include');
  } finally {
    globalThis.fetch = originalFetch;
    delete globalThis.localStorage;
...
test('graph assist storage persists per session and maps to request mode', () => {
  const store = new Map();
  globalThis.localStorage = {
  ...
  try {
    assert.equal(getGraphAssistEnabled('session-a'), false);
    setGraphAssistEnabled('session-a', true);
    assert.equal(getGraphAssistEnabled('session-a'), true);
    assert.equal(getGraphAssistEnabled('session-b'), false);
    assert.equal(graphRetrievalModeForEnabled(true), 'graph_assist');
    assert.equal(graphRetrievalModeForEnabled(false), 'standard');
  } finally {
    delete globalThis.localStorage;
  }
});
...
test('querySessionStream parses NDJSON chunks and invokes callbacks', async () => {
  ...
  try {
    ...
    assert.equal(calledOptions.method, 'POST');
    assert.equal(JSON.parse(calledOptions.body).graph_retrieval_mode, 'graph_assist');
  } finally {
    globalThis.fetch = originalFetch;
    delete globalThis.localStorage;
  }
